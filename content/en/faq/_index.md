...
indicates the contents of the node are given in the GIF file `yourface.gif`. The [image]({{< relref "/docs/attrs/image.md" >}}) attribute specifies which file to use. 
(There is also the deprecated [shapefile]({{< relref "/docs/attrs/shapefile.md" >}}) 
...
The [page]({{< relref "/docs/attrs/page.md" >}}) attribute, if set, tells Graphviz to print the graph as an array of pages of the given size. Thus, the graph
...
See also the [viewport]({{< relref "/docs/attrs/viewport.md" >}}) attribute.
...
Use the `-Tcmapx` command line option. See [here](/docs/outputs/imap/) for more details.
...
neato internally supports layouts in higher dimensions through the [dim]({{< relref "/docs/attrs/dim.md" >}}) and [dimen]({{< relref "/docs/attrs/dimen.md" >}}) attributes, 
...
Use the graph attribute [overlap]({{< relref "/docs/attrs/overlap.md" >}}).
